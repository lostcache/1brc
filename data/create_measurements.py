...
def get_weather_station_file_path():
    script_dir = get_script_dir()
    weather_station_file_path = script_dir / "weather_stations.csv"
    return weather_station_file_path


def get_measurements_file_path():
    script_dir = get_script_dir()
    measurements_file_path = script_dir / "measurements.txt"
...
def build_weather_station_name_list():
    ...
    """
    station_names = []
    with open(get_weather_station_file_path(), "r") as file:
...
def build_test_data(weather_station_names, num_rows_to_create):
    ...
    try:
        with open(get_measurements_file_path(), "w") as file:
            progress = 0
    ...
    file_size = os.path.getsize(get_measurements_file_path())
    human_file_size = convert_bytes(file_size)

    print(f"Test data successfully written to {get_measurements_file_path()}")
